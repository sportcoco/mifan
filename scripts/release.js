...
async function release() {
  ...
  if (!args.publishOnly) {
    ...
    await exec(lernaCli, ['version', '--exact', '--no-commit-hooks', '--no-git-tag-version', '--no-push']);
    ...
    Object.keys(rootPkg.devDependencies).forEach(name => {
      if (name.startsWith('@mifanjs/') && !name.startsWith('@mifanjs/p')) {
        rootPkg.devDependencies[name] = currVersion;
      }
    });
    writeFileSync(join(__dirname, '..', 'package.json'), JSON.stringify(rootPkg, null, 2) + '\n', 'utf-8');
  ...
  const url = newGithubReleaseUrl({
    repoUrl: 'https://github.com/assets/mifan',
    tag,
    body: changelog,
    isPrerelease: isNext
  });
